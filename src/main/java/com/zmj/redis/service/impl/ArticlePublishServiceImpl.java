...
public class ArticlePublishServiceImpl implements ArticlePublishService {
    ...
    public AjaxResult articlePublish(Article article) {
        ...
        redisTemplate.opsForZSet().addIfAbsent(articleTimeQueueKey, article.getAuthor(), publishTime);
        //将文章加入到SortSet K:文章ID V:文章发布者 Score:文章分数(评分)
        redisTemplate.opsForZSet().addIfAbsent(articleScoreQueueKey, article.getAuthor(), publishTime + Article.SCORE_PER_VOTE);
